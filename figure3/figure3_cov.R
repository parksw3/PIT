...
g1 <- ggplot(ss_AB) +
	geom_line(data=ss_A, aes(time/52-tmax, prevalence1, col="More fit"), lwd=1) +
	geom_line(aes(time/52, prevalence1, col="More fit"), lwd=1) +
	geom_line(aes(time/52, prevalence2, col="Less fit"), lwd=1) +
...
figure4_cov <- ggarrange(g1, g2, nrow=1)

ggsave("figure3_cov.pdf", figure4_cov, width=5, height=3)
